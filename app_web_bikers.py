...
try:
    df = pd.read_excel(FILE_EXCEL)
    # Assicuriamoci che ogni riga abbia un ID (per vecchi dati)
    ...
    for idx, row in df.iterrows():
        ...
        with st.expander(f"{row['Data']} - {row['Nome Evento / Raduno']}"):
            ...
            # Modifica Protetta
            pwd = st.text_input(f"Password per modificare", type="password", key=f"p_{event_id}")
            if pwd == "Judaz2026":
                new_info = st.text_area(f"Modifica Info", value=str(row.get('Dettagli / Note', '')), key=f"edit_{event_id}")
                if st.button("SALVA MODIFICHE", key=f"save_{event_id}"):
                    df.loc[df['ID'] == event_id, 'Dettagli / Note'] = new_info
                    df.to_excel(FILE_EXCEL, index=False)
                    st.rerun()
        ...
        if ha_gia_votato(event_id):
            st.button(label, key=f"btn_{event_id}", disabled=True)
        else:
            if st.button(label, key=f"btn_{event_id}"):
                df.loc[df['ID'] == event_id, 'Partecipanti'] = conteggio + 1
                df.to_excel(FILE_EXCEL, index=False)
                registra_voto(event_id)
                st.rerun()
